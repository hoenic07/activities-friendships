...
require "activities_friendships/use_case/activity/index"
require "activities_friendships/use_case/friendship/index"
...
class App < Sinatra::Base
  get '/activities' do
    ...
    collection = ActivitiesFriendships::UseCase::Activity::Index.new(filters: filters).run
    Api::ResponseHandler::Collection.new(collection).to_rack_response
  end
  
  get '/users/:user_id/friendships' do |user_id|
    collection = ActivitiesFriendships::UseCase::Friendship::Index.new(user_id: user_id).run
    Api::ResponseHandler::Collection.new(collection).to_rack_response
  end
